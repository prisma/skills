...
| `$transaction(...)` — works on v6 with a replica set ([v6 docs](https://www.prisma.io/docs/orm/overview/databases/mongodb#replica-set-configuration)) | The 0.16 façade does not expose `db.transaction(...)`, **but the underlying `mongodb` driver is directly available** (user-supplied peer dependency): multi-document atomicity works through driver sessions (`client.startSession()` / `session.withTransaction(...)`) on a replica set | Do not promise a façade delivery date |
